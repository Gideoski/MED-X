...
export default function Level300Page() {
  ...
  const lvl300Categories = useMemo(() => categories?.filter(c => c.level === 300) || [], [categories]);

  const freeRef = useMemoFirebase(() => (firestore ? collection(firestore, 'materials_300lvl_free') : null), [firestore]);
  const premiumRef = useMemoFirebase(() => (firestore ? collection(firestore, 'materials_300lvl_premium') : null), [firestore]);
  ...
  const allEbooks = useMemo(() => {
    const free = freeEbooks ? freeEbooks.map(e => ({ ...e, id: e.title, collection: 'materials_300lvl_free' })) : [];
    const prem = premiumEbooks ? premiumEbooks.map(e => ({ ...e, id: e.title, collection: 'materials_300lvl_premium' })) : [];
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="text-center space-y-2">
        <h1 className="text-4xl font-bold tracking-tight">300-Level Study Hub</h1>
        <p className="text-muted-foreground text-lg">Clinical and advanced materials for high-performing students.</p>
      </div>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-primary/80 px-1">
            <Filter className="h-4 w-4" /> Filter by Clinical Course
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 border rounded-3xl bg-muted/20">
          <p className="text-muted-foreground text-lg">No materials found for this level.</p>
